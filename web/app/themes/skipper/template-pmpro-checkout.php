<?php
/**
 * Template Name: PMPro Checkout Template
 */
?>
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
		...
		<tr id="other_discount_code_tr" style="display: none;">
			<td>
				<div>
					...
					<input id="other_discount_code" name="other_discount_code" type="text" class="form-control  <?php echo pmpro_getClassForField("other_discount_code");?>" size="20" value="<?php echo esc_attr($discount_code)?>" />
	...
	?>
<div class="skipper-pmpro-wrap">
	<?php if(!$skip_account_fields && !$pmpro_review) { ?>
	<table id="pmpro_user_fields" class="pmpro_checkout table" width="100%" cellpadding="0" cellspacing="0" border="0">
	<thead>
		<tr>
			<th>
	...
	<tbody>
		<tr>
			<td>
				<div class="form-group col-sm-12">
					...
					<input id="username" name="username" type="text" class="form-control  <?php echo pmpro_getClassForField("username");?>" size="30" value="<?php echo esc_attr($username)?>" />
				</div>
        ...
        <div class="row">
  				<div class="form-group col-sm-6">
  					...
  					<input id="password" name="password" type="password" class="form-control  <?php echo pmpro_getClassForField("password");?>" size="30" value="<?php echo esc_attr($password)?>" />
  				</div>
  				<?php
  					...
  					<div class="form-group col-sm-6">
  						...
  						<input id="password2" name="password2" type="password" class="form-control  <?php echo pmpro_getClassForField("password2");?>" size="30" value="<?php echo esc_attr($password2)?>" />
  					</div>
          </div>
					<?php
					}
        ...
        <div class="row">
				...
				<div class="pmpro_hidden">
					...
					<input id="fullname" name="fullname" type="text" class="form-control  <?php echo pmpro_getClassForField("fullname");?>" size="30" value="" /> <strong><?php _e('LEAVE THIS BLANK', 'pmpro');?></strong>
				</div>
	...
	<?php if(pmpro_getGateway() == "paypal" && empty($pmpro_review) && true == apply_filters('pmpro_include_payment_option_for_paypal', true ) ) { ?>
		<table id="pmpro_payment_method" class="pmpro_checkout top1em table" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling) { ?>style="display: none;"<?php } ?>>
		<thead>
			<tr>
	...
	<?php
		...
		{
	?>
	<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em table" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_billing_address_fields", false) ){ ?>style="display: none;"<?php } ?>>
	<thead>
		<tr>
	...
	<tbody>
        ...
        <div class="row">
  				<div class="form-group col-sm-6">
  					...
  					<input id="bfirstname" name="bfirstname" type="text" class="form-control  <?php echo pmpro_getClassForField("bfirstname");?>" size="30" value="<?php echo esc_attr($bfirstname)?>" />
  				</div>
  				<div class="form-group col-sm-6">
  					<label for="blastname"><?php _e('Last Name', 'pmpro');?></label>
  					<input id="blastname" name="blastname" type="text" class="form-control  <?php echo pmpro_getClassForField("blastname");?>" size="30" value="<?php echo esc_attr($blastname)?>" />
  				</div>
        </div>
				<div class="form-group col-sm-12">
					...
					<div class="form-group row">
						...
						<?php
							...
							if($state_dropdowns === true || $state_dropdowns == "names")
							{
								global $pmpro_states;
							?>
							<select name="bstate" class="form-control <?php echo pmpro_getClassForField("bstate");?>">
								<option value="">--</option>
								<?php
        ...
        <div class="row">
  				<div class="form-group col-sm-6">
  					...
  					<select name="bcountry" class="form-control  <?php echo pmpro_getClassForField("bcountry");?>">
  						<?php
  				...
  				<div class="form-group col-sm-6">
  					...
  					<input id="bphone" name="bphone" type="text" class="form-control  <?php echo pmpro_getClassForField("bphone");?>" size="30" value="<?php echo esc_attr(formatPhone($bphone))?>" />
  				</div>
        </div>
	...
	<?php
		...
		{
		?>
		<table id="pmpro_payment_information_fields" class="table pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_payment_information_fields", false) ) { ?>style="display: none;"<?php } ?>>
		<thead>
			<tr>
				<th>
			...
			<tr valign="top">
				<td>
					...
					<?php
						...
						{
						?>
						<div class="pmpro_payment-card-type form-group row">
							<label for="CardType"><?php _e('Card Type', 'pmpro');?></label>
							<select id="CardType" name="CardType" class="form-control  <?php echo pmpro_getClassForField("CardType");?>">
					...
					<div class="pmpro_payment-account-number form-group row">
					...
					<div class="pmpro_payment-expiration form-group row">
						...
						</select> /<select id="ExpirationYear" name="ExpirationYear" class="form-control  <?php echo pmpro_getClassForField("ExpirationYear");?>">
							<?php
					...
					<?php
						...
						if($pmpro_show_cvv) { ?>
					<div class="pmpro_payment-cvv form-group row">
						<label for="CVV"><?php _e('CVV', 'pmpro');?></label>
						<input class="form-control " id="CVV" name="CVV" type="text" size="4" value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class="<?php echo pmpro_getClassForField("CVV");?>" />  <small><a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL)?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e("* (what's this?)", 'pmpro');?></a></small>
					</div>
					<?php } ?>

					<?php if($pmpro_show_discount_code) { ?>
					<div class="pmpro_payment-discount-code form-group row">
						...
						<input type="button" id="discount_code_button" name="discount_code_button" class="btn btn-primary" value="<?php _e('Apply', 'pmpro');?>" />
	...
	<?php
		...
		{
		?>
		<table id="pmpro_tos_fields" class="pmpro_checkout top1em table" width="100%" cellpadding="0" cellspacing="0" border="0">
		<thead>
		<tr>
		...
		<tbody>
			<tr class="odd">
				<td>
					<div id="pmpro_license form-group row">
...
<!--
	...
	jQuery('.pmpro_error').bind("change keyup form-control ", function() {
...
jQuery("#AccountNumber, .pmpro_payment-cvv input").addClass('form-control').removeClass('input');
jQuery(".pmpro_payment-expiration, .pmpro_payment-cvv").addClass("ccdetails form-group");
jQuery(".pmpro_payment-account-number").addClass("form-group col-sm-6");
jQuery(".ccdetails").addClass('col-sm-6');
jQuery(".ccdetails").wrapAll('<div class="row">');
jQuery('#ExpirationMonth, #ExpirationYear').addClass('expirationGroup');
jQuery('.expirationGroup').wrapAll('<div class="row wrapExpiration"></div>');
jQuery('.expirationGroup').wrap('<div class="col-xs-6 innerWrap"></div>');
jQuery("#ExpirationMonth, #ExpirationYear, .pmpro_payment-cvv input").addClass('form-control');
jQuery('.pmpro_payment-expiration').contents().filter(function() { return this.nodeType == Node.TEXT_NODE; }).remove();
jQuery('.pmpro_payment-cvv input').before(jQuery('.pmpro_payment-cvv small'));
//jQuery('.pmpro_payment-discount-code').addClass('row');
jQuery('#discount_code').addClass('form-control discountGroup').removeClass('input');
jQuery('#discount_code_button').addClass('btn btn-prima discountGroup');
jQuery('.discountGroup').wrapAll('<div class="row discountWrap"></div>');
jQuery('.discountGroup').wrap('<div class="col-xs-6"></div>');
jQuery('.pmpro_payment-discount-code').addClass('form-group col-sm-12');
jQuery('.pmpro_payment-cvv small').addClass('pmpro_asterisk');
jQuery('.form-group').each(function() {
  jQuery(this).find('label, .pmpro_asterisk').wrapAll('<div class="row"></div>');
});
jQuery('td > .form-group').wrap('<div class="row"></div>');
</script>
